...
class DetallesTapizados(models.Model):
    ...
    cantidad_telasColores = models.IntegerField(default=0)
    cantidad_tipoMaterial = models.IntegerField(default=0)

    def __str__(self):
        return f"{self.tipoTela.nombre} - {self.color.nombre} - {self.producto.descripcion} - {self.tipoEspumilla.nombre}"